...
def on_article_delete(instance, *args, **kwargs):
    ...
    def get_lost_and_found(org=None):
        ...
        urlpath_root_args = {'org': org} if org else {}
        try:
            ns.lost_and_found = URLPath.objects.get(
                slug=settings.LOST_AND_FOUND_SLUG, parent=URLPath.root(**urlpath_root_args), site=site
            )
        except URLPath.DoesNotExist:
            article = Article(
                group_read=True, group_write=False, other_read=False, other_write=False, organization=org,
            )
            ...
            ns.lost_and_found = URLPath.objects.create(
                ...
                parent=URLPath.root(**urlpath_root_args),
                site=site,
                article=article,
            )
